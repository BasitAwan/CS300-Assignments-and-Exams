...
func lookup(codes []int, palette [][3]uint8) []uint8 {
    ...
    for i := 0; i < len(codes); i++ {
        for j := 2; j >=0; j-- {
            output[3*i+(2-j)]= palette[codes[i]][j]
        }
    }
    return output
}
...
In this part, you are given a list of GIF codes and you will produce color codes that are then used by the lookup function you wrote in part 1/2. 
You will get a dictionary (actually a 2D array) that you need to index using the GIF codes. However, unlike parts 1/2 the number of color codes against a GIF code is variable.
 So the size of the resulting array is not known in advance. To avoid repeated allocations (with make or append), you are required to first calculate the size of the output array 
 (by going over all GIF codes and finding the length of color codes against each GIF code) and then do a single allocation (using make) and finally traverse to fill in the color codes.
*/
func decompress(codes []int, dict [][]int) []int {
    size:=0
    for i:= range codes{
        for j:= range dict[codes[i]]{
            j=j
            size++
        }
    }
...
In this part, you are to parallelize the above GIF decoding. It is easy to divide the task (like Part 2) but the complication is that we do not know where should the goroutine working on the 
right half of GIF codes start writing in the output array. Thus we use a PrefixSum kind of solution. We ask both halves how much space they need and then tell the total to the parent. 
The parent will then give a big enough slice. Since the left half told us how much space it will occupy, we can send the correct sub-slices to the left and right goroutines.
Like part 2, our sequential cut-off will be 16 and the given Printf statement will help you see which tasks are generated.
...
func parallel_decompress(codes []int, dict[][]int, size chan<- int, slice <-chan []int) {
    ...
    // size<- 100 // tell parent size
    // <-slice // get slice from parent
    ...
    }else 
    {
        ...
        go parallel_decompress(codes1, dict ,size1, slice1)
        go parallel_decompress(codes2, dict ,size2, slice2)
        sizerecv1 := <-size1
        sizerecv2 := <-size2
        totalsize := sizerecv1 + sizerecv2 
        size <- totalsize
        colors:= <- slice
        colors1:= colors[:sizerecv1]
        colors2:= colors[sizerecv1:]
        // colors1 := make([]int, sizerecv1)
        // colors2 := make([]int, sizerecv2)
        slice1 <- colors1
        slice2 <-colors2
        <-size1
        <-size2
        // index :=0 
        // for i := range colors1 {
        //     colors[index] = colors1[i]
        //     index++
        // }
        // for i := range colors2 {
        //     colors[index] = colors2[i]
        //     index++
        // }
