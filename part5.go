...
In this part, you are given a list of GIF codes (just like parts 3/4) and you will produce the GIF dictionary which was gien to you in parts 3/4. Dictionary generation is very simple. 
Initially the dictionary contains 258 entries. Entries 0-255 contain a 1-color array containing the corresponding color code e.g. an array containing just 0 at index 0, an array containing 
just 1 at index 1 and so on. The next two entries contain empty arrays. 

After that, for each of the GIF codes (EXCLUDING the first two), you will create a new dictionary entry containing ALL colors from the dictionary entry of the PREVIOUS code followed by ONE 
new color code:
...
func dictionary(codes []int) [][]int {
    ...
    for i:= range codes{
        ...
        {
            element:= make([]int,0)
            for j:= range dict[codes[i-1]]{
                element = append(element,dict[codes[i-1]][j])
            }
            if codes[i]<len(dict) {
                element = append(element,dict[codes[i]][0])

            }else
            {
                element = append(element,dict[codes[i-1]][0])
            }
...
You are to only divide the work in just two halves in this part (not recursively). We will also set the maximum dictionary size to exactly 4096 entries. The problem in this part is that 
dictionary entries on the right may depend on dictionary entries on the left that have not yet been written. Our solution is to make the dictionary an array of channels. 
You will start 4096 goroutines running the 'once' function (given below) for each channel in the dictionary.
 You can see that this goroutine will block anyone reading from the channel until a value is written and then it will indefinitely respond to readers.
  Now the goroutine handling the right half of the GIF codes can easily start its working as any value not available will block it until it becomes available. 
  Make helper functions or lambda functions as the actual goroutines for the left and right halves.
...
func left(codes []int, dict [][2]chan []int )  [][2]chan []int{
    ...
    for i := 0; i < check ; i++{
        ...
        {
            element:= make([]int,0)
            values:= <-dict[codes[i-1]][1]
            for j:= range values{
                element = append(element,values[j])
            }

            if codes[i] < index {
                values1:= <-dict[codes[i]][1]
                element = append(element,values1[0])

            }else
            {
                element = append(element,values[0])
            }
            dict[index][0] <-element
            index++
        }
        
    }
    for i:= index+ check; i < 4096 ; i++{
        element:= make([]int,0)
        dict[i][0] <- element
        element = <-dict[i][1]
    }
...
func right(codes []int, dict [][2]chan []int )[][2]chan []int  {
    check := len(codes)/2
    index:=258 + check 
    for i:= check; i<len(codes) ; i++ {
        if i<2 {
        }else
        {
            element:= make([]int,0)
