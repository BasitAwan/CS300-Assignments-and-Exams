...
In this part, you are given a list of codes where each code refers to a color on the color pallete (a 2D array). Each color in the color palette is stored as an array of Red, Green,
 and Blue components. Your task is to lookup the palette for each color code and return an array containing RGB colors in the opposite order (i.e. Blue, then Green and then Red) because
  BMP format stores them in the opposite order. If the input array has 100 codes, your output array will have 300 color values. You do not have to handle out-of-range values.
*/
...
Parallelize the above function such that both halves of the codes array are recursively given to separate goroutines. There should be no subdivision for arrays of length 16 or less 
(sequential cut-off). The Printf statement written in the function can help you see which tasks are being generated. Make sure that both halves are running in goroutines (and not just sequential
 recursive calls) and wait for both halves to terminate using a channel.
...
func parallel_lookup(codes []int, palette [][3]uint8, colors []uint8, done chan<- bool) {
    ...
    if len(codes) <=16{
        for i := 0; i < len(codes); i++ {
            for j := 2; j >=0; j-- {
                colors[3*i+(2-j)]= palette[codes[i]][j]
            }
        }
        done <-true
    }else 
    {
        ...
        codes1:= codes[:split]
        codes2:= codes[split:]
        colors1 := colors[:split2]
        colors2 := colors[split2:]
        done1 := make(chan bool)
        done2 := make(chan bool)
        go parallel_lookup(codes1, palette, colors1, done1)
        go parallel_lookup(codes2, palette, colors2, done2)
        <-done1
        <-done2
        done <-true
