...
socket.on('game', data => {
	if (data[0]==usernum) {
		message = 'It\'s your move click on second grid to send attack'
		setState()
		attackClick=true
	}
	else if
	{
		message = 'Waiting for opponnents move'
		setState()
		attackClick=false
...
const makeBox2 = (x, y ,ID) => React.createClass({
	render: () =>{
		return React.createElement('div', {id:ID, className: checkBox2(x,y), onClick: ()=>opClick})
	}
})
